...
function InsertModele() {
    ...
    const loadMarque =async ()=>{
        const result=await axios.get("http://cloudwebservice-production-7ad7.up.railway.app/marques", {
            headers: {
    ...
    const loadCarburant =async ()=>{
        const result=await axios.get("http://cloudwebservice-production-7ad7.up.railway.app/carburants", {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
